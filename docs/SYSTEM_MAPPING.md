...
## Update Rule
When stock/checklist/narcotic inventory Excel files change, run `npm run generate:data` and `npm run validate:data`. The hospital drug source is fixed to the exact path `약제팀 라벨/원내보유의약품리스트.xlsx` so similarly named matching workbooks cannot be selected. When it changes, run both `npm run generate:data` and `npm run generate:labels`. When `약제팀 라벨/원내보유의약품_라벨매칭_20260702.xlsx` changes, run `npm run generate:label-matches`. When narcotic label or conversion workbooks in `마약/` change, refresh their generated JSON. The UI should update from generated data without editing React components.
