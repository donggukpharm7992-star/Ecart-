...
const source = join(root, "병동라벨", "원내보유의약품리스트.xlsx");
const generatedFiles = [
  join(root, "src", "data", "inventory.generated.json"),
  join(root, "src", "data", "narcoticInventory.generated.json"),
  join(root, "병동라벨", "data", "hospitalDrugLabels.generated.json"),
];
const deployDir = join(root, ".deploy");
const statePath = join(deployDir, "ward-drug-autosync-state.json");
const lockPath = join(deployDir, "ward-drug-autosync.lock");
const logPath = join(deployDir, "ward-drug-autosync.log");
const taskName = "HospitalWardDrugLabelAutoSync";
const pollMs = 5000;
...
function syncAndRelease() {
  ...
  runNpm(["run", "generate:ward-labels"]);
  runNpm(["run", "release"]);
